...
class Memory(nn.Module):
    ...
    def forward(self, query, value=None, inference=False, cha_first=False):
        ...
        device = query.device
        tr_fusion, recon_loss, contrastive_loss = None, torch.zeros(1, device=device), torch.zeros(1, device=device)
        ...
        # Update
        if not inference:
            B, S, D = value.shape  # Get dimensions directly from value
            ...
            contrastive_loss = 0.5 * torch.abs(
                torch.eye(self.slot, device=value_norm.device) -
                torch.matmul(value_norm, value_norm.transpose(0, 1))
            ).sum()    #n_slot,n_slot
