...
class CTCDecoder(nn.Module):
    def __init__(self, input_size, vocab_size, hidden_size=512):
        ...
        # CRITICAL FIX: Much stronger bias initialization against blank
        # This forces the model to explore non-blank outputs initially
        with torch.no_grad():
            # Strong negative bias for blank to prevent collapse
            self.fc.bias[0] = -2.0  # Strongly discourage blank
            # Positive bias for actual characters
            self.fc.bias[1:] = 0.5  # Encourage non-blank classes
            
            # Also initialize weights with smaller scale for better gradients
...
class SentenceDecoder(nn.Module):
    def __init__(self, vocab_size=29, radius=8.0, n_slot=112, head=8):  # Default radius reduced
        super().__init__()
        ...
        self.ctc_decoder = CTCDecoder(
            input_size=256 * len([3, 5, 7]) * 1,  # 768
    ...
    def forward(self, x, aud, infer=False, mode='tr'):
        """
        Args:
            x: (B, T, 512) - input features
            aud: (B, T, 512) - audio features (can be None during inference)
            infer: bool - inference mode
            mode: 'tr' or 'te' - training/test mode
        Returns:
            logits: (B, T, vocab_size)
            recon: reconstruction loss
            const: contrastive loss
        """
        # TCN expects (B, C, T) so transpose
        x_trans = x.transpose(1, 2)  # (B, 512, T)
        tcn_out, recon, const = self.tcn(x_trans, aud, infer, mode)
        tcn_out = tcn_out.transpose(1, 2)  # (B, T, 768)
        
        logits = self.ctc_decoder(tcn_out)  # (B, T, vocab_size)
        
        return logits, recon, const